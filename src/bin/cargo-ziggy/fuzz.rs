...
impl Fuzz {
	...
	pub fn fuzz(&mut self) -> Result<(), anyhow::Error> {
		let build =
			Build { no_afl: !self.afl(), no_honggfuzz: !self.honggfuzz(), release: self.release };
		...
		if Path::new(&self.corpus()).exists() {
			if self.minimize {
				...
				self.copy_corpora().context("Could not move all seeds to temporary corpus")?;
				let _ = fs::remove_dir_all(self.corpus_minimized());
				self.run_minimization().context("Failure while minimizing")?;
		...
		loop {
			...
			let crash_dirs = glob(&format!("{}/afl/*/crashes", self.output_target()))
				...
				.flatten()
				.chain(vec![format!("{}/honggfuzz/{}", self.output_target(), self.target).into()]);
			...
			if self.afl() && last_sync_time.elapsed().as_secs() > 10 {
				let afl_corpus =
					glob(&format!("{}/afl/mainaflfuzzer/queue/*", self.output_target(),))?
						.flatten();
			...
			if processes.iter_mut().all(|p| p.try_wait().unwrap_or(None).is_some()) {
				stop_fuzzers(&mut processes)?;
	...
	pub fn spawn_new_fuzzers(&self) -> Result<Vec<process::Child>, anyhow::Error> {
		...
		if afl_jobs > 0 {
			...
			for job_num in 0..afl_jobs {
				...
				// Power schedule
				let power_schedule =
					afl_modes.get(job_num as usize % afl_modes.len()).unwrap_or(&"fast");
				// Old queue cycling
		...
		if honggfuzz_jobs > 0 {
			let hfuzz_help = process::Command::new(&cargo)
				...
				.env("HFUZZ_WORKSPACE", format!("{}/honggfuzz", self.output_target()))
			...
			eprintln!("{} honggfuzz              ", style("    Launched").green().bold());
		}
		...
		if honggfuzz_jobs > 0 {
			eprintln!(
				"  {}",
				style(format!("tail -f {}/logs/honggfuzz.log", self.output_target())).bold()
			);
		}
		...
		self.all_seeds()?.iter().for_each(|s| {
			let _ = fs::copy(
				...
				format!(
					"{}/{}",
					&self.corpus_tmp(),
					s.file_name().unwrap_or_default().to_str().unwrap_or_default(),
				),
			);
		});
		Ok(())
	...
	pub fn print_stats(&self) {
		...
		if !self.afl() {
			...
			let afl_stats_process = process::Command::new(cargo)
				.args(["afl", "whatsup", "-s", &format!("{}/afl", self.output_target())])
				.output();
		...
		if !self.honggfuzz() {
			...
			let hf_stats_process = process::Command::new("tail")
				.args(["-n300", &format!("{}/logs/honggfuzz.log", self.output_target())])
				.output();
				...
				for raw_line in s.split('\n') {
					...
					if let Some(total_execs) = line.strip_prefix("Iterations : ") {
						...
						hf_new_finds = String::from(
							hf_new_finds.strip_prefix("0 days ").unwrap_or(&hf_new_finds),
						);
						hf_new_finds = String::from(
							hf_new_finds.strip_prefix("00 hrs ").unwrap_or(&hf_new_finds),
						);
						hf_new_finds = String::from(
							hf_new_finds.strip_prefix("00 mins ").unwrap_or(&hf_new_finds),
						);
						hf_new_finds = String::from(
		...
		let mut total_run_time = time_humanize::HumanTime::from(self.start_time.elapsed())
			.to_text_en(time_humanize::Accuracy::Rough, time_humanize::Tense::Present);
...
pub fn kill_subprocesses_recursively(pid: &str) -> Result<(), Error> {
	let subprocesses = process::Command::new("pgrep").arg(&format!("-P{pid}")).output()?;
