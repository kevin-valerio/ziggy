...
fn kill_subprocesses_recursively(pid: &str) {
	let subprocesses = process::Command::new("pgrep").arg(&format!("-P{pid}")).output().unwrap();
...
#[test]
fn integration() {
	let unix_time = format!("{}", SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs());
	...
	assert!(temp_dir_path.join("url-fuzz").join("coverage").join("index.html").is_file());
	...
	assert!(plot.success());
	assert!(temp_dir_path.join("url-fuzz").join("plot").join("index.html").is_file());
}
